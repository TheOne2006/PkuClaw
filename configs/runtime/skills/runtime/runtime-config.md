...
configs/runtime/
  ...
  runtime.json      # 不提交；Agent provider、Codex 参数、loop 配置、通知策略
...
首次本地运行前，从 `runtime.example.json` 复制出 `runtime.json`。不要提交
`runtime.json`，也不要把启动期凭据、飞书密钥、系统包安装、账号密码、OTP
或私有 channel 目标标识写入可提交文件。
...
修改本地 `runtime.json` 的 `loops[]`。需要保留：
...
所有 loop 共用的默认通知目标写在本地 `runtime.json` 的 `notifications` section：
