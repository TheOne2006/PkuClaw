...
本文描述当前 runtime / prompt / skill / 通知脚本架构。
...
```text
source = loop
suggested_skills = loop.skill_names + (tools/channel-notifier.md,)
```
...
configs/runtime/
  ...
  skills/               # skill markdown 文件
    ...
    pku3b/              # pku3b 安装和使用说明
    tasks/              # 课程/学习任务
    tools/              # 通用辅助能力与 loop 通知脚本说明
```

如果 Agent 需要查看或修改 runtime，应直接读写这些文件。启动期凭据、飞书密钥、绑定 host/port 等不属于热 runtime，不应被 Agent 随意修改。
...
`user_request`、`loop_id`、`notification_script_skill` 等。要改身份、规则、
...
不得包含 run id、source、Agent settings、provider/model/reasoning、repository root、run directory、runtime config path、Recent Runs、通知脚本、完整 skill 正文等内部实现细节。
...
scripts/        # loop 通知脚本 thin clients
```
...
后台 loop 主动通知用户时使用文件队列：

```text
loop Agent -> channel-notifier skill -> Python queue script -> data/notify_queue/*.json -> daemon queue worker -> CoreRuntime -> Feishu/backend
```

脚本层只负责把 JSON job 写入共享队列目录：

- `python scripts/pkuclaw_notify.py text --message "..."`
- `python scripts/pkuclaw_notify.py card --card-file card.json`
- `python scripts/pkuclaw_notify.py image --image-path image.png`（daemon 当前返回 unsupported）
- `python scripts/pkuclaw_notify.py update-card --card-id ... --card-file card.json --sequence N`

脚本不访问 localhost HTTP，不直连飞书，不解析 runtime target，不做卡片渲染；这些由 daemon/CoreRuntime 和 channel backend 统一负责。daemon 默认约每 5 秒扫描一次队列，并写入 ack 文件供脚本确认。
...
Agent 只传通知内容；loop id 由 provider 通过环境变量传给脚本，daemon 根据 loop id 自动解析覆盖目标或全局默认目标。未配置时发送会失败并提示配置缺失。
...
- loop prompt 指向 Notification Script Skill；
